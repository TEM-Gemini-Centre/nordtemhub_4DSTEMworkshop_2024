...
def unwrap_trotter(phase, mask):

    mask = mask.astype(int)
    if np.sum(mask>0):
        inv_mask = np.ones_like(mask)-mask
        phase_masked = np.ma.array(phase, dtype=np.float32, mask=inv_mask)
        unwrapped_phase = unwrap(phase_masked)
        unwrapped_phase[mask==0] = 0

        if np.mean(unwrapped_phase[mask==1])>np.pi:
            unwrapped_phase[mask==1] -= 2*np.pi
        if np.mean(unwrapped_phase[mask==1])<=-np.pi:
            unwrapped_phase[mask==1] += 2*np.pi
...
def Phase_compen(single_trotter,coefficients,theta_x,theta_y,scan_angles_x,scan_angles_y,Aperture,wave_len, return_AA=False):
    ...
    AA_plus_true_phase=unwrap_trotter(np.angle(AA_plus), mask=LTrotter[0])
    AA_plus_true_phase[LTrotter[0]==0]=0

    AA_minus_true_phase=unwrap_trotter(np.angle(AA_minus), mask=RTrotter[0])
