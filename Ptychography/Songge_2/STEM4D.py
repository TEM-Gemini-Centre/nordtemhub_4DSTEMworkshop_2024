...
class Data4D():
    ...
    def plot_aperture(self):
        ...
        fig, ax = self._init_figure(1,3, (12,3))
        im=ax[0].imshow(np.log(Ronchi_norm*1e4+1),cmap=cm.viridis)
        ...
        ax[2].plot(aperture_round[0],aperture_round[1], linewidth=3) 
...
# The center of the new ronchigram should be close to the center pixel
    ...
    def plot_4D_reciprocal_both(self, log=False):
        try:
            import hyperspy.api as hs
        except:
            hs = None
            ...
            print('4D plot not possible')
            return
        data_4D_Reciprocal_abs = self.data_4D_Reciprocal.copy()
        data_4D_Reciprocal_abs[int(self.power_spectra.shape[0]/2)-1:int(self.power_spectra.shape[0]/2)+1,
                               int(self.power_spectra.shape[1]/2)-1:int(self.power_spectra.shape[1]/2)+1,:,:] *= 1/50
        if log:
            c = np.stack((np.log(np.abs(data_4D_Reciprocal_abs)+1), (np.angle(self.data_4D_Reciprocal))), axis=0)
        else:
            c = np.stack((np.abs(data_4D_Reciprocal_abs)+1, (np.angle(self.data_4D_Reciprocal))), axis=0)
    ...
    def plot_4D_reciprocal(self,signal = 'amplitude',log=True):
        ...
        if signal == 'amplitude':
            if log:
                s = hs.signals.Signal2D(np.log(np.abs(self.data_4D_Reciprocal)+1))
            else:
    ...
    def plot_FT(self):
        fig, ax = self._init_figure(1,2,(12,4))
    ...
    def plot_trotter(self, frame):
        row,col = frame
        fig, ax = self._init_figure(1,2,figsize=(12,4))
    ...
    def plot_trotters(self, selected_frames = None, plot_constrains = True, skip = 0):       
        rotation_angle = self.rotation_angle
        ind=np.unravel_index(np.argsort(self.power_spectra, axis=None), self.power_spectra.shape) # rank from low to high 
        
        fig, ax = self._init_figure(2,3,(12,6))
        fig2, ax2 = self._init_figure(2,3,(12,6))
        self.selected_frames = []
        for i in range(6):
            # take 6 bright spots in fft as a example
            ...
            ax[int(i/3),i%3].set_title('phase index '+str(frame_y_idx)+' '+str( frame_x_idx))
            ax[int(i/3),i%3].set_axis_off()
            ...
            ax2[int(i/3),i%3].set_title('amplitude index '+str(frame_y_idx)+' '+str( frame_x_idx))
            ax2[int(i/3),i%3].set_axis_off()
        plt.show()
    ...
    def plot_higher_order_trotters(self,rotation_angle,selected_frames = None, order = 1,log = True, plot_constrains = True, skip = 0):       
        ...
        ind=np.unravel_index(np.argsort(self.power_spectra, axis=None), self.power_spectra.shape) # rank from low to high 
        self.fig_trotters_phase, ax = self._init_figure(3,3,(10,10))
        self.fig_trotters_amp, ax2 = self._init_figure(3,3,(10,10))
        self.selected_frames = []
        for i in range(9):
...
class SVD_AC():
    def __init__(self, data4D, trotters_nb=6):
        self.data4D = data4D
    ...
    def find_trotters(self,skip=0):
        ...
        #take 6 bright spots in fft as a default
        ...
        print('trotter index: ' + str(self.data4D.selected_frames))
    ...
    def run_svd(self,iterations = 50, coefficients = [], cf=0.5, order=3, aberr_order_seq=[]):
        ...
        for itt in range(iterations):
            ...
            for i in range(self.trotters_nb):
                ...
                ST_phase_unwrap=unwrap_trotter(ST_phase, mask=Trotter[0])
    ...
    def plot_corrected_trotters(self,frames,aberrations):
        ...
        for i in range(len(frames)):
            ...
            self.corrected_trotters.append([np.angle(single_trotter),np.angle(Trotter_correction)])
        
        fig, ax = self.data4D._init_figure(2,len(self.corrected_trotters), (len(self.corrected_trotters)*2, 3),
...
class SSB():
    ...
    def integrate_trotter(self,LTrotterSum,scan_x_idxs,scan_y_idxs):
        for scan_x_idx in scan_x_idxs:
            print('Progress ' +str(int(np.round(scan_x_idx/self.data4D.scan_col*100 + 1)))+' %\t', end='\r')
    ...
    def integrate_trotter_AC(self, LTrotterSum, scan_x_idxs, scan_y_idxs, aberrations, order):
        
        func_aberr = Aberr_func(self.data4D.Ronchi_angles_x, self.data4D.Ronchi_angles_y, aberrations, order)
        func_transfer = np.exp(-1j*2*np.pi/(self.data4D.wave_len*1e-10)*func_aberr)
        dx,dy = np.meshgrid(self.data4D.Ronchi_angles_x, self.data4D.Ronchi_angles_y)
        d_zero = np.sqrt(dx*dx+dy*dy)
        ...
        for scan_x_idx in scan_x_idxs:
            ...
            for scan_y_idx in scan_y_idxs:
                ...
                Ronchi_x_plus, Ronchi_y_plus = self.data4D.Ronchi_angles_x + scan_x_angle, self.data4D.Ronchi_angles_y + scan_y_angle
                Ronchi_x_minus, Ronchi_y_minus = self.data4D.Ronchi_angles_x - scan_x_angle, self.data4D.Ronchi_angles_y - scan_y_angle

                dx,dy = np.meshgrid(Ronchi_x_plus, Ronchi_y_plus)
                d_plus = np.sqrt(dx*dx+dy*dy)
                dx,dy = np.meshgrid(Ronchi_x_minus, Ronchi_y_minus)
                d_minus = np.sqrt(dx*dx+dy*dy)
                
                LTrotter_mask = np.ones(single_trotter.shape)
                if scan_x_idx != np.fix(self.data4D.scan_col/2) or scan_y_idx != np.fix(self.data4D.scan_row/2):
                    LTrotter_mask[d_minus>=self.data4D.aperturesize] = 0
                    LTrotter_mask[d_zero>=self.data4D.aperturesize] = 0
                    LTrotter_mask[d_plus<self.data4D.aperturesize] = 0
                ...
                # compensate phase
                # CTF -Q
                func_aberr_minusQ = Aberr_func(Ronchi_x_minus, Ronchi_y_minus, aberrations, order)
                func_transfer_minusQ = np.exp(-1j*2*np.pi/(self.data4D.wave_len*1e-10)*func_aberr_minusQ)
                AA_minus_phase = np.angle(np.conj(func_transfer)*func_transfer_minusQ)
        
                LTrotter_phase_compen = LTrotter_phase + AA_minus_phase
                LTrotter_phase_compen[LTrotter_mask==0] = 0
                LTrotter = LTrotter_amp*np.exp(1j*(LTrotter_phase_compen))
                LTrotter[LTrotter_mask==0] = 0
                Lpixel_num =np.sum(LTrotter_mask)
                if Lpixel_num ==0:
                    LTrotterSum[scan_y_idx,scan_x_idx] = 0
                else:
    ...
    def integrate_trotter_higher_order(self,LTrotterSum,scan_x_idxs,scan_y_idxs,order = 2):
        ...
        for scan_x_idx in scan_x_idxs:
            print('Progress ' +str(int(scan_x_idx/self.data4D.scan_row*100+1))+' %\t', end='\r')
            for scan_y_idx in scan_y_idxs: 
                scan_x_angle = self.data4D.scan_angles_x[scan_x_idx]*np.cos(self.data4D.rotation_angle) - self.data4D.scan_angles_y[scan_y_idx]*np.sin(self.data4D.rotation_angle)
                scan_y_angle = self.data4D.scan_angles_x[scan_x_idx]*np.sin(self.data4D.rotation_angle) + self.data4D.scan_angles_y[scan_y_idx]*np.cos(self.data4D.rotation_angle)
                single_trotter=self.data4D.data_4D_Reciprocal[scan_y_idx,scan_x_idx,:,:]
...
#            self.integrate_trotter_higher_order(self.LTrotterSum,scan_x_idxs,scan_y_idxs,order=order)
            if len(aberrations) == 0:
                ...
                self.integrate_trotter_AC(self.LTrotterSum, scan_x_idxs, scan_y_idxs, -aberrations, order)
    ...
    def plot_result(self, sample=1, write_file=False):
        self.fig,ax = self.data4D._init_figure(1,2,(12,4))
        if sample >1:
...
class WDD():
    ...
    def run(self,aberrations=np.zeros(25),epsilon = 0.01):
        ...
        for scan_y_idx in range (self.data4D.scan_row):
            print('WDD probe generation progress',int((scan_y_idx+1)/self.data4D.scan_row*100),'%',end='\r')
...
class iCoM():
    ...
    def run(self,sigma = 0):
        ...
        self.icom = self.calc_CoM(self.data4D.data_4D, self.data4D.BFdisk, 
                                  self.data4D.center_x, self.data4D.center_y, self.data4D.scan_angle_step_x)
    ...
    def plot_result(self, write_file=False):
        ...
        self.ic = self.icom[0]+self.icom[1]
        im0 = ax[0].imshow(self.ic, extent= [0, self.ic.shape[0]*self.data4D.step_size, 0, self.ic.shape[1]*self.data4D.step_size])
        self.fig.colorbar(im0,ax=ax[0])        
        ax[0].set_title('iCoM')
        im1 = ax[1].imshow(np.log(np.abs(np.fft.fftshift(np.fft.fft2(self.ic)))), 
                           extent = [0, self.ic.shape[0]*self.data4D.step_size, 0, self.ic.shape[1]*self.data4D.step_size])
        self.fig.colorbar(im1, ax=ax[1])        
        ax[1].set_title("FFT")
        if write_file == True:
